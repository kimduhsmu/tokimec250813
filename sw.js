...
self.addEventListener('install', (event) => {
  event.waitUntil(
    (async () => {
      ...
      // 일부 파일이 배포 경로에 없더라도 설치가 실패하지 않도록 개별 캐시합니다.
      for (const filePath of FILES_TO_CACHE) {
        try {
          await cache.add(filePath);
        } catch (error) {
          console.warn('[Service Worker] 캐시 생략:', filePath, error);
        }
      }
      // 즉시 활성화 단계로 넘어갑니다.
      self.skipWaiting();
...
self.addEventListener('fetch', (event) => {
  ...
  const isHtmlRequest = request.mode === 'navigate' || (isSameOrigin && (url.pathname.endsWith('/') || url.pathname.endsWith('.html')));
